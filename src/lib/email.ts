...
// ============================================================
// ONLINE CURSUS — aankoop bevestiging (klant) + verkocht-notificatie (Chiva)
// ============================================================
export interface CoursePurchaseMailData {
  email: string
...
export async function sendCourseBevestigingMail(data: CoursePurchaseMailData) {
  try {
    ...
    const { error } = await resend.emails.send({
      from: FROM,
      to: data.email,
      subject: 'Je hebt toegang — veel plezier met de cursus!',
      html: `<!DOCTYPE html>
<html lang="nl" xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<title>Je hebt toegang</title>
<!--[if mso]><noscript><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch></o:OfficeDocumentSettings></xml></noscript><![endif]-->
</head>
<body style="margin:0; padding:0; background-color:#e8e6e1; -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%;">
<div style="display:none; max-height:0; overflow:hidden; opacity:0; mso-hide:all;">Je LUXIQUE cursus staat klaar.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#e8e6e1;">
  <tr><td align="center" style="padding:40px 16px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#FAF8F4; border-radius:14px; overflow:hidden;">
      <tr><td align="center" style="background-color:#0C0A07; padding:38px 40px 30px 40px;">
      ...
      <tr><td style="height:2px; line-height:2px; font-size:0; background-color:#C4A265;">&nbsp;</td></tr>
      <tr><td style="padding:44px 48px 36px 48px;" align="center">
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:11px; letter-spacing:3px; text-transform:uppercase; color:#C4A265; padding-bottom:18px;">Aankoop bevestigd</div>
        <div style="font-family:'Cormorant Garamond', Georgia, 'Times New Roman', serif; font-size:34px; line-height:42px; font-weight:500; color:#0C0A07; padding-bottom:20px;">Je hebt toegang!</div>
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:16px; line-height:26px; color:#4a463e; padding-bottom:24px; max-width:440px; margin:0 auto;">Bedankt voor je aankoop${data.voornaam ? ', ' + data.voornaam : ''}! Je cursus staat klaar in je account — je kunt meteen beginnen.</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3efe7; border-radius:10px; margin:0 0 26px 0;">
          <tr><td style="padding:22px 26px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr><td style="font-family:Arial,sans-serif; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; color:#9a958b; padding:0 0 3px 0;">Cursus</td></tr>
              <tr><td style="font-family:'Cormorant Garamond',Georgia,serif; font-size:19px; color:#0C0A07; padding:0 0 14px 0;">${data.cursus_naam}</td></tr>
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" style="margin:6px auto 0 auto;">
          <tr>
            <td align="center" bgcolor="#C4A265" style="border-radius:9px; background:linear-gradient(180deg,#D8B978,#C4A265);">
              <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="${startUrl}" style="height:52px;v-text-anchor:middle;width:280px;" arcsize="17%" fillcolor="#C4A265" stroke="f"><center style="color:#0C0A07;font-family:Arial,sans-serif;font-size:15px;font-weight:bold;">Start met de cursus</center></v:roundrect><![endif]-->
              <!--[if !mso]><!--><a href="${startUrl}" style="display:inline-block; font-family:Arial, Helvetica, sans-serif; font-size:15px; font-weight:bold; letter-spacing:.5px; color:#0C0A07; text-decoration:none; padding:17px 44px; border-radius:9px;">Start met de cursus &rarr;</a><!--<![endif]-->
            </td>
          </tr>
        </table>
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:13px; line-height:21px; color:#8a857b; padding-top:18px; max-width:430px; margin:0 auto;">Vragen tijdens de cursus? Mail <a href="mailto:info@luxique.nl" style="color:#8a857b; text-decoration:underline;">info@luxique.nl</a> — reactie binnen 2 werkdagen.</div>
      </td></tr>
      <tr><td style="padding:0 48px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="height:1px; line-height:1px; font-size:0; background-color:#e4ddd0;">&nbsp;</td></tr></table></td></tr>
      <tr><td align="center" style="padding:26px 48px 34px 48px;">
        <div style="font-family:'Cormorant Garamond', Georgia, serif; font-style:italic; font-size:18px; color:#C4A265; padding-bottom:14px;">With love, Luxique</div>
    ...
    if (error) {
      console.error('Cursus-bevestiging: fout:', error)
      return
    }
    console.log(`✅ Cursus-bevestiging verzonden naar ${data.email}`)
  } catch (err) {
    console.error('Cursus-bevestiging: onverwachte fout:', err)
  }
}

export async function sendCourseVerkochtNotificatie(data: { klant_naam: string; klant_email: string; cursus_naam: string; bedrag: number }) {
  try {
    const { error } = await resend.emails.send({
      from: FROM,
      to: CHIVA_EMAIL,
      subject: `You've sold a course! ${data.cursus_naam}`,
      html: `<!DOCTYPE html>
<html lang="nl" xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<title>You've sold a course!</title>
<!--[if mso]><noscript><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch></o:OfficeDocumentSettings></xml></noscript><![endif]-->
</head>
<body style="margin:0; padding:0; background-color:#e8e6e1; -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%;">
<div style="display:none; max-height:0; overflow:hidden; opacity:0; mso-hide:all;">Nieuwe cursusverkoop binnengekomen.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#e8e6e1;">
  <tr><td align="center" style="padding:40px 16px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#FAF8F4; border-radius:14px; overflow:hidden;">
      <tr><td align="center" style="background-color:#0C0A07; padding:38px 40px 30px 40px;">
      ...
      <tr><td style="padding:44px 48px 36px 48px;" align="center">
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:11px; letter-spacing:3px; text-transform:uppercase; color:#C4A265; padding-bottom:18px;">You've sold a course!</div>
        <div style="font-family:'Cormorant Garamond', Georgia, 'Times New Roman', serif; font-size:34px; line-height:42px; font-weight:500; color:#0C0A07; padding-bottom:20px;">${data.klant_naam}</div>
        <div style="font-family:Arial, Helvetica, sans-serif; font-size:16px; line-height:26px; color:#4a463e; padding-bottom:24px; max-width:440px; margin:0 auto;">Er is zojuist een online cursus verkocht en betaald:</div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3efe7; border-radius:10px; margin:0 0 26px 0;">
          <tr><td style="padding:22px 26px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr><td style="font-family:Arial,sans-serif; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; color:#9a958b; padding:0 0 3px 0;">Klant</td></tr>
              ...
              <tr><td style="font-family:Arial,sans-serif; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; color:#9a958b; padding:0 0 3px 0;">Cursus</td></tr>
              <tr><td style="font-family:'Cormorant Garamond',Georgia,serif; font-size:19px; color:#0C0A07; padding:0 0 14px 0;">${data.cursus_naam}</td></tr>
              <tr><td style="font-family:Arial,sans-serif; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; color:#9a958b; padding:0 0 3px 0;">Ontvangen (Stripe)</td></tr>
              <tr><td style="font-family:'Cormorant Garamond',Georgia,serif; font-size:19px; color:#C4A265; padding:0 0 14px 0;">&euro;${data.bedrag.toFixed(2).replace('.', ',')}</td></tr>
            </table>
          </td></tr>
        </table>
      </td></tr>
    ...
    if (error) {
      console.error('Cursus-verkocht notificatie: fout:', error)
      return
    }
    console.log(`✅ Cursus-verkocht notificatie verzonden naar ${CHIVA_EMAIL}`)
  } catch (err) {
    console.error('Cursus-verkocht notificatie: onverwachte fout:', err)
  }
}
