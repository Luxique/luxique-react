...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'enrollments' | 'courses'>('users')
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <a href="/dashboard" className="text-[12px] text-[#888] hover:text-[#D4AF37] transition px-3 py-1.5 rounded-full border border-[#eee]">← Dashboard</a>
                ...
                {profiles.map(p => (
                  <tr key={p.id} className="border-b border-[#f5f5f5] hover:bg-[#fafafa]">
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${p.role === 'admin' ? 'bg-[#D4AF37]/10 text-[#D4AF37]' : 'bg-[#f5f5f5] text-[#888]'}`}>{p.role}</span>
                    </td>
                    <td className="px-5 py-3 flex gap-2">
                      <button onClick={() => toggleRole(p.id, p.role)} className="text-[11px] px-3 py-1 rounded-full border border-[#ddd] hover:border-[#D4AF37] text-[#888] hover:text-[#D4AF37] transition">
                        {p.role === 'admin' ? 'Demote' : 'Admin'}
                      </button>
                      <button onClick={() => { setGrantUserId(p.id); setShowGrant(true) }} className="text-[11px] px-3 py-1 rounded-full bg-[#D4AF37] text-white hover:bg-[#C5A028] transition">
                        + Cursus
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'enrollments' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-['Cormorant_Garamond'] text-[20px]">Alle inschrijvingen</h2>
              <button onClick={() => setShowGrant(true)} className="text-[12px] px-4 py-2 rounded-full bg-[#D4AF37] text-white font-semibold hover:bg-[#C5A028] transition">
                + Handmatig toewijzen
              </button>
            </div>
            <div className="bg-white rounded-2xl border border-[#eee] overflow-hidden">
                ...
                <tbody>
                  {enrollments.map(e => (
                    <tr key={e.id} className="border-b border-[#f5f5f5] hover:bg-[#fafafa]">
                      <td className="px-5 py-3 text-[13px]">{e.profiles?.full_name || e.profiles?.email || '—'}</td>
                      <td className="px-5 py-3 text-[13px]">{e.courses?.title || '—'}</td>
                      ...
                      <td className="px-5 py-3">
                        <button onClick={() => revokeAccess(e.id)} className="text-[11px] px-3 py-1 rounded-full border border-red-200 text-red-400 hover:bg-red-50 transition">
                          Intrekken
                        </button>
                      </td>
                    </tr>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select value={grantUserId} onChange={e => setGrantUserId(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl border border-[#ddd] text-[14px] focus:outline-none focus:border-[#D4AF37]">
                    <option value="">Selecteer cursist...</option>
                    {profiles.map(p => (
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setShowGrant(false)} className="flex-1 py-3 rounded-full border border-[#eee] text-[13px] text-[#888] hover:border-[#ddd] transition">
                    Annuleren
                  </button>
                  <button onClick={grantAccess} disabled={granting || !grantUserId || !grantCourseId}
                    className="flex-1 py-3 rounded-full bg-[#D4AF37] text-white font-semibold text-[13px] hover:bg-[#C5A028] transition disabled:opacity-50">
