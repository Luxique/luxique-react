...
export async function POST(request: NextRequest) {
  ...
  const manualHasLedger = source === 'manual' && isManualAvailabilityLedger(booking.availability_restoration_ledger)
  const finalUpdate = source === 'manual'
    ? {
        status: 'cancelled', cancelled_at: requestedAt, cancelled_within_24h: within24h,
        cancellation_requested_at: requestedAt,
        sync_status: manualHasLedger ? 'availability_restore_pending' : 'availability_review_required',
        sync_error: manualHasLedger ? null : 'Bestaande boeking zonder betrouwbare beschikbaarheidsledger; handmatige controle vereist.',
  ...
  if (source === 'manual' && manualHasLedger) {
    try {
      ...
      warnings.push('De afspraak is geannuleerd; beschikbaarheidsherstel wordt apart opgevolgd.')
    }
  } else if (source === 'manual') {
    warnings.push('De afspraak is geannuleerd; de beschikbaarheid moet handmatig worden nagekeken.')
  }
  try {
    if (source === 'online') {
