...
export async function GET(request: NextRequest) {
  ...
  const [{ data: online, error: onlineError }, { data: manual, error: manualError }] = await Promise.all([
  ...
  if (onlineError || manualError) return NextResponse.json({ error: onlineError?.message || manualError?.message }, { status: 500 })
  ...
  for (const booking of manual || []) {
    ...
    try {
      ...
      results.push({ source: 'manual', id: booking.id, status: 'cancelled' })
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Onbekende fout'
