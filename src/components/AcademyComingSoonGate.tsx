...
interface AcademyComingSoonGateProps {
  children: React.ReactNode
  scope?: 'page' | 'section'
}

export default function AcademyComingSoonGate({ children, scope = 'page' }: AcademyComingSoonGateProps) {
  ...
  const sectionScoped = scope === 'section'

  return (
    <div className={`relative ${sectionScoped ? '' : 'min-h-screen'}`}>
      <div aria-hidden={blocked} className={blocked ? 'pointer-events-none select-none blur-[12px]' : ''}>
        {children}
      </div>
      {blocked && (
        <div className={`${sectionScoped ? 'absolute' : 'fixed'} inset-0 z-40 flex items-start justify-center bg-[#0C0A07]/35 px-5 pt-24 backdrop-blur-[10px]`} role="status">
