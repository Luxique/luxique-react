...
ALTER TABLE courses ADD COLUMN IF NOT EXISTS differentiators jsonb DEFAULT $json$[
...
UPDATE courses SET
  hero_badge_text = COALESCE(NULLIF(hero_badge_text, ''), title, ''),
  hero_title = COALESCE(NULLIF(hero_title, ''), title, ''),
  hero_tagline = COALESCE(NULLIF(hero_tagline, ''), description, '')
